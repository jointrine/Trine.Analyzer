...
namespace SolutionFixer
{
    static class Program
    {
        ...
        {
            Console.WriteLine("MSBuildLocator.RegisterDefaults");
            MSBuildLocator.RegisterDefaults();

            using var workspace = MSBuildWorkspace.Create();
            Console.WriteLine("OpenSolutionAsync");
            var solution = await workspace.OpenSolutionAsync(solutionPath);

            // var updatedSolution = await new InjectCodeWriter("EventBus", "_eventBus").Inject(workspace, solution);
            var updatedSolution = await RunCodeFixers(solution);

            workspace.TryApplyChanges(updatedSolution);
        }

        private static async Task<Solution> RunCodeFixers(Solution solution)
        {
        ...
        private static async Task<Document> AnalyzeDocumentAsync(Document document, IEnumerable<Diagnostic> diags,
